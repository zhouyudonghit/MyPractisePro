...
public class DragSortListView extends ListView {
    private String TAG = DslvTestActivity.TAG_PREFIX+getClass().getSimpleName();
    ...
    //added by zhouyd for that pnly certain area can be draged
    private int startDragIndex = 0;
    private int endDragIndex;
    private Context mContext;

    //是否开启可拖动范围控制
    private boolean isIndexControl = true;

    public void setIsIndexControl(boolean flag){
        this.isIndexControl = flag;
    }
    ...
    private boolean updatePositions() {
        ...
        Log.d(TAG,"updatePositions(),firstVisiblePosition = "+first+",startPos = "+mFirstExpPos);
        if (startView == null) {
    ...
    private void measureFloatView() {
        if (mFloatView != null) {
            ...
            Log.d(TAG,"measureFloatView(),mFloatView.getMeasuredHeight() = "+mFloatView.getMeasuredHeight()+"mFloatView.getMeasuredWidth() = "+mFloatView.getMeasuredWidth());
    ...
    protected boolean onDragTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                float x = ev.getX();
                float y = ev.getY();
                int touchPos = pointToPosition((int)x, (int)y);
                //if( touchPos < getStartDragIndex() || touchPos > getEndDragIndex())
                if( touchPos > getEndDragIndex() && isIndexControl)
                {
                    //cancelDrag();
                    //stopDrag(false);
                    mDragScroller.stopScrolling(true);
    ...
    public boolean startDrag(int position, int dragFlags, int deltaX, int deltaY) {
        Log.d(TAG,"startDrag,position = "+position+",dragFlags = "+dragFlags+"deltaX = "+deltaX+"deltaY = "+deltaY);
    ...
    private void doDragFloatView(boolean forceInvalidate) {
        ...
        Log.d(TAG,"getFirstVisiblePosition() = "+getFirstVisiblePosition()+",getChildCount() = "+getChildCount());
    ...
    private void updateFloatView() {
        ...
        if (mFloatViewManager != null) {
            ...
            Log.d(TAG,mFloatLoc.toString()+","+mTouchLoc.toString());
        }
        ...
        Log.d(TAG,"updateFloatView(),mFloatLoc.x = "+mFloatLoc.x+",mFloatLoc.y = "+mFloatLoc.y);
