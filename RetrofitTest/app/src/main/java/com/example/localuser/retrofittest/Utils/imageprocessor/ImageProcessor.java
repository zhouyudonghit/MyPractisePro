...
public class ImageProcessor implements Handler.Callback {
  ...
  public static final String CACHE_UPLOAD_IMG_DISK_ROOT_DIR = FileStorageUtil.getDiskCacheDir().getPath() + "/upload_img_disk_cache/";
  ...
  private static File getImageCacheDir(Context context) {
    File cacheDir = FileStorageUtil.getDiskCacheDir();
    if (cacheDir != null) {
      File result = new File(CACHE_UPLOAD_IMG_DISK_ROOT_DIR);
  ...
  /**
   * connect asynchronous compress thread
   */
        ...
        public void run() {
          try {
            ...
            if(result!=null){
              ...
              String msg="path is empty";
              Throwable tr=new Throwable(msg);
              mHandler.sendMessage(mHandler.obtainMessage(MSG_COMPRESS_ERROR, tr));
            }
          } catch (IOException e) {
  ...
  /**
   * connect compress and return the file
   */
  ...
  public static class Builder {
    ...
    public Builder load(final Uri uri) {
      mStreamProviders.add(new InputStreamProvider() {
        ...
        @Override
        public String getPath() {
          return FileStorageUtil.getImagePath(uri,null);
        }
      });
      return this;
    }
